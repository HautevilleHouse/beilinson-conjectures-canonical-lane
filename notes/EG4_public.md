...
In-paper anchor: `paper/BEILINSON_CONJECTURES_PREPRINT.md` (`BEI_G4`, `BEI_G5`).

## Goal
Separate the rigidity job from the endpoint-transfer job for `proving the regulator and special-value package for motives through an admissible motivic closure architecture`.
...
- endpoint-transfer lock: `value_transfer`.
- coherence interface: `eps_coh` remains available to the bridge and final margin.

## Closure Criterion

`BEI_G4` closes when `rho_rigidity` excludes bad endpoint alternatives: bad regulator/L-value countermodels are excluded.
`BEI_G5` closes when `value_transfer` transfers the surviving endpoint to the intended target class: rigid limit transfers to the motivic special-value endpoint class.
Together they feed the strict margin `M_BEI`.
...
The surviving rigid representative is locked to the standard problem-side endpoint by `value_transfer`.

Payload: read this note together with `notes/IDENTIFICATION_BRIDGE.md`.

### Theorem EG4.3 (rigidity/transfer gate closure)
If bad limits are excluded and the endpoint lock is active, then `BEI_G4` and `BEI_G5` deliver the boundary object needed by the final margin.

## Current Instantiation

- rigidity gate: `BEI_G4`
- rigidity artifact key: `rho_rigidity`
- transfer gate: `BEI_G5`
