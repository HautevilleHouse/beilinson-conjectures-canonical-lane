...
In-paper anchor: `paper/BEILINSON_CONJECTURES_PREPRINT.md` (`BEI_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving the regulator and special-value package for motives through an admissible motivic closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`BEI_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and regulator windows do not collapse.
This is the first-failure capture contribution to `M_BEI`.
...
If near-failure extraction and no-Zeno spacing hold, then `BEI_G3` prevents hidden drift before rigidity is applied.
