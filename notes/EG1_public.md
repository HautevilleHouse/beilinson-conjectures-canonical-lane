...
In-paper anchor: `paper/BEILINSON_CONJECTURES_PREPRINT.md` (`BEI_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving the regulator and special-value package for motives through an admissible motivic closure architecture`.
...
`BEI_G1` closes when `kappa_regulator` satisfies the response-floor requirement: projected regulator response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_BEI`.
...
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_regulator`, then `BEI_G1` supplies the projected/protected-core input to `M_BEI`.
