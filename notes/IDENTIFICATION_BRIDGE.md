...
## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving the regulator and special-value package for motives through an admissible motivic closure architecture`.
...
2. the endpoint-transfer lock `value_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_BEI` remains positive.
...
Payload: verify `value_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_BEI` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `value_transfer`
